...
init python:
    ...
    splash_message_default = "Це неофіційна модифікація, котра ніяк не зв'язано з Team Salvato."
...
label splashscreen:
    ...
    if not persistent.Firstrun:
        ...
        with Dissolve(1.0)
        pause 1.0
        "Це неофіційна модифікація до гри Doki Doki Literature Club, котре ніяк не зв'язано з Team Salvato."
        "Перед проходженням данної модифікації необхідно пройти пройти оригінальну гру для уникнення спойлерів."
        "Завантажити гру можна безкоштовно в Steam або за посиланням: http://ddlc.moe"
    ...
    if persistent.first_run and (config.version == persistent.oldversion or persistent.autoload == "postcredits_loop"):
        ...
        menu:
            "Попередній файл збережень було знайдено. Хочете все видалити й почати заново?"
            "Так, видалити всі існуючі данні." if True:
                "Видалення...{nw}"
                python:
                    delete_all_saves()
                    ...
                    renpy.utter_restart()
            "НІ, продовжити де був продолжить там же." if True:
    ...
    if not persistent.first_run:
        ...
        "[config.name] — це фанатське доповнення до гри Doki Doki Literature Club, яке жодним чином не пов’язане з Team Salvato."
        "Це означає, що ви вже пройшли оригінальну гру, адже тут містяться певні спойлери."
        "Для проходження цього доповнення вам знадобляться файли оригінальної Doki Doki Literature Club, які ви можете завантажити за посиланням: https://ddlc.moe або у Steam."

        menu:
            "Граючи у [config.name], ви погоджуєтесь з тим, що вже пройшли оригінальну гру та готові до будь-яких спойлерів, які тут містяться."
            "Я згоден." if True:
                pass
        ...
        if extra_settings:
            if process_check(["obs32.exe", "obs64.exe", "obs.exe", "xsplit.core.exe", "livehime.exe", "pandatool.exe", "yymixer.exe", "douyutool.exe", "huomaotool.exe"]):
                ...
                call screen dialog("Режим Let's Play було автоматично увімкнено.\nЦей режим дозволяє пропускати контент, який\nмістить чутливі/дорослі сцени, \nабо змінювати налаштування історії.\n\nЦе налаштування залежить від того, чи увімкнув\nавтор модифікації ці прапорці у своїй історії.\n\n Щоб вимкнути режим Let's Play, зайдіть у Налаштування і\nзніміть прапорець з опції «Режим Let's Play».",
                    [Hide("dialog"), Return()])
        scene white
    ...
    $ config.main_menu_music = "mod_assets/Sound/Fireplace-LoFi.ogg"
    $ renpy.music.play(config.main_menu_music)
