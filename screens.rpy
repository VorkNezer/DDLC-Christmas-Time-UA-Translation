...
define -3 text_outline_color = "#660000"
...
init -1 style window:
    ...
    background Transform("mod_assets/gui/mc-textbox.png", xalign=0.5, yalign=1.0)
        ...
        for i in items:
            ...
            if "kwargs=" in i.caption:
                ...
                if "#" in kwarg:
                    ...
                    textbutton caption:
                        idle_background Frame(im.MatrixColor(im.MatrixColor("mod_assets/choice_idle_background.png", im.matrix.desaturate() * im.matrix.contrast(1.29) * im.matrix.colorize("#00f", "#fff") * im.matrix.saturation(120)), 
                            im.matrix.desaturate() * im.matrix.colorize(arg1, arg2)), gui.choice_button_borders)
                        hover_background Frame(im.MatrixColor(im.MatrixColor("mod_assets/choice_hover_background.png", im.matrix.desaturate() * im.matrix.contrast(1.29) * im.matrix.colorize("#00f", "#fff") * im.matrix.saturation(120)), 
...
init -1 style choice_button is default:
    ...
    idle_background Frame("mod_assets/choice_idle_background.png", gui.choice_button_borders)
    hover_background Frame("mod_assets/choice_hover_background.png", gui.choice_button_borders)
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    ...
                    textbutton _("Нова гра") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Введите своё имя", ok_action=Function(FinishEnterName)))

            else:

                textbutton _("Історія") action [ShowMenu("history"), SensitiveIf(renpy.get_screen("history") == None)]

                textbutton _("Зберегти гру") action [ShowMenu("save"), SensitiveIf(renpy.get_screen("save") == None)]

            textbutton _("Завантажити гру") action [ShowMenu("load"), SensitiveIf(renpy.get_screen("load") == None)]
            ...
            if _in_replay:
                ...
                if persistent.playthrough != 3:
                    ...
                    textbutton _("Головне меню") action NullAction()

            textbutton _("Налаштування") action [ShowMenu("preferences"), SensitiveIf(renpy.get_screen("preferences") == None)]
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Допомога") action OpenURL("https://t.me/VorkNezer")


                textbutton _("Вихід") action Quit(confirm=not main_menu)
        else:
...
init -1 style navigation_button_text:
    ...
    font "mod_assets/comic.ttf"
    color "#fff"
    outlines [(4, "#660000", 0, 0), (2, "#660000", 2, 2)]
...
init -501 screen file_slots(title):
    ...
    default page_name_value = FilePageNameInputValue(pattern=u'Сторінка {}')
...
init -1 style slot_button:
    ...
    idle_background Frame("mod_assets/slot_idle_background.png", gui.choice_button_borders)
    hover_background Frame("mod_assets/slot_hover_background.png", gui.choice_button_borders)
            ...
            hbox:
                ...
                if renpy.variant("pc"):
                    ...
                    vbox:
                        style_prefix "radio"
                        label _("Режим")
                        textbutton _("Вікно") action Preference("display", "window")
                        textbutton _("Повноекранний") action Preference("display", "fullscreen")
                if config.developer:
                    vbox:
                        style_prefix "radio"
                        label _("Сторона відкату")
                        textbutton _("Відключити") action Preference("rollback side", "disable")
                        textbutton _("Зліва") action Preference("rollback side", "left")
                        textbutton _("Зправа") action Preference("rollback side", "right")

                vbox:
                    style_prefix "check"
                    label _("Пропустити")
                    textbutton _("Непрочитанне") action Preference("skip", "toggle")
                    textbutton _("Після вибору") action Preference("after choices", "toggle")
                ...
                if extra_settings:
                    vbox:
                        style_prefix "check"
                        label _("Додатково")
                        textbutton _("Без цензури") action If(persistent.uncensored_mode, 
                            ToggleField(persistent, "uncensored_mode"), 
                            Show("confirm", message="Ви дійсно хочете ввімкнути режим без цензури?\nЦя дія дозволить вам дорослий/чутливий\nконтент в вашому проходжені.\n\nЦе налаштування залежить від того, чи ввімкнув\nавтор модифікації ці галочки в його історію.", 
                        ...
                        textbutton _("Let's Play") action If(persistent.lets_play, 
                            ...
                            [ToggleField(persistent, "lets_play"), Show("dialog", 
                                message="Ви ввімкнули режим Let's Play.\nЦе налаштування дозволить вам пропускати контент, котрий\nмістить чутливий/дорослий контент,\nабо змінити налаштування історії.\n\nЦе налаштування залежить від того, чи ввімкнув\nавтор модифікації ці галочки в його історії.", 
...
init -1 style confirm_frame:
    background Frame("mod_assets/frame.png", gui.confirm_frame_borders, tile=gui.frame_tile)
